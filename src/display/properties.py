...
class PropertyApp(Plotter):
    ...
    def initial_request(self):
        ...
        if '2024' not in self.selected_year:
            ...
            self.properties_input = fetch_data_BigQuery(bigquery_cred, self.selected_department)
        ...
        if not self.properties_input is None:
            ...
            if self.normalize_by_area:
                self.properties_input['valeur_fonciere'] = (self.properties_input['valeur_fonciere'] / self.properties_input['surface_reelle_bati']).round().astype(int)
